public class Polynome {
    ...
    public Polynome(double a, int n) {
        if (Math.abs(a) > EPS) // Если основание степени больше нуля, создаём объект монома
            head = new Monome(a, n);
    }
    ...
    public Polynome(Monome otherMon) {
        ...
        while (otherMon.next != null) { // Пока полином с которого копируем не кончился
            temphead.next = new Monome(otherMon.next); // Копируем следующий элемент списка

            // Движемся вперёд по полиномам
    ...
    // Умножение текущего полинома на число, ВОЗВРАЩАЕТ РЕЗУЛЬТАТ
    public Polynome multiply(double a){
        // Если число ноль или полином нулевой, вернуть null
        if (Math.abs(a) <= EPS || head == null)
            return null;

        // Полином ненулевой
        Polynome res = new Polynome(head.a * a, head.n); // Создание полинома-результата,
        Monome m2 = res.head;                   // Указатель по собираемому полиному
        double val;                             // Получаемое значение
        // Проход по всем коэффициентам и умножение, пока текущий полином не закончится
        for (Monome m1 = head; m1.next != null; m1 = m1.next) {
            val = m1.next.a * a;                // Значение полученное после умножения
            if (Math.abs(val) > EPS) {          // Если получился не ноль добавляем элемент после m2
                insert(m2, new Monome(val, m1.next.n));
                m2 = m2.next;                   // Если добавили, то сдвигаем указатель, иначе нет
            }
        }
        return res;
    }
    ...
    public void multiply(Polynome pal) {
        ...
        if (pal.head == null || head == null) {
            head = null;
            return;
        }
        // Создание головы нового связного списка-результата
        Monome res_head = new Monome(pal.head.a * head.a, pal.head.n + head.n);
        // Переменные ссылки по всем трём полиномам m1 m2 m3
        Monome m1 = head, m2 = pal.head.next, m3;
        ...
        // Проход по элементам текущего полинома
         while (m1 != null){
            ...
            while (m2 != null){
                pow = m1.n + m2.n;
                if (m3.next == null || m3.next.n < pow)   // Дошли до конца, самая меньшая степень или следующее слагаемое имеет степень меньше. | вставляем после текущего m3
                    insert(m3, new Monome(m1.a * m2.a, pow));
                else if (m3.next.n == pow) {  // Степени равны, коэффициенты складываются
                    m3.next.a += m1.a * m2.a;
                    if (Math.abs(m3.next.a)<=EPS) // Если получился ноль, удаляем элемент
                        delete(m3);
                }
                else {// если ничего не сделали со списком, идём дальше по нему
                    m3 = m3.next; // Идём дальше
                    continue; // Мы не ушли с элемента второго списка т.к. ищем место в 3м
                }

                m2 = m2.next; // Продолжаем идти по второму списку
            }
            // Переход к умножению на следующим множитель из первого полинома
            m1 = m1.next;
            m2 = pal.head;
    }
        head = res_head; // Замена старого полинома на получившийся
    }

    // Метод складывает текущий полином с входящим
    public void sum(Polynome pal){
        if (pal == null){ // Если на вход ничего нет, не с чем складывать, ничего не изменяется
            return;
        }
        ...
        while (m1 != null && m2 != null) { // Пока не дойдём до конца хотябы одного из полиномов
            if (m1.n == m2.n) { // Если степени равны, сложить коэффициенты
                ...
                if (Math.abs(coeff) > EPS) {
                    insert(m3, new Monome(coeff, m1.n));
    ...
    public void print() {
        for (Monome t = head; t != null; t = t.next) { // Проход по всему связному списку
            ...
            System.out.printf("(%d) ", t.n); // Показатель степени
        }
        System.out.println();
    }
